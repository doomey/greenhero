       ...
       function selectOrCreateUser(connection, callback) {
          ...
          connection.query(select, [googleId], function(err, results) {
             if(err) {
                ...
                if(results.length === 0) {
                   var insert = "insert into iparty(nickname, google_id, google_token, google_email, totalleaf, partytype, registration_token, google_name) "+
                                "values(?, ?, ?, ?, 0, 1, ?, " +
                                sqlAes.encrypt(1) +
                                //"aes_encrypt(?, unhex(" + connection.escape(serverKey) + ")) " +
                                ")";
                   connection.query(insert, [(!parseToken.payload.nickname ? parseToken.payload.name : parseToken.payload.nickname),
                       googleId, req.body.id_token, parseToken.payload.email, req.body.registration_id, parseToken.payload.name], function(err, result) {
                      connection.release();
                      if(err) {
                         callback(err);
                      } else {
